
namespace DropMath
{
    inline float& Vec3::operator[](int i)
    {
        DM_ASSERT(i >= 0 && i < 3);
        return array[i];
    }
    inline const float& Vec3::operator[](int i) const
    {
        DM_ASSERT(i >= 0 && i < 3);
        return array[i];
    }
    inline bool Vec3::operator==(const Vec3& v) const { return IsZero(x - v.x) && IsZero(y - v.y) && IsZero(z - v.z); }
    inline bool Vec3::operator!=(const Vec3& v) const { return !(*this == v); }

    inline float Vec3::Length() const { return Sqrt(LengthSquared()); }

    inline void Vec3::Normalize()
    {
        float len = Length();
        if (len > F::EPSILON)
        {
            x /= len;
            y /= len;
            z /= len;
        }
    }

	inline Vec3 Vec3::Normalized() const
	{
        Vec3 v = *this;
		v.Normalize();
		return v;
	}


    DM_CONSTEXPR inline Vec3 Vec3::Lerp(const Vec3& a, const Vec3& b, float t) { return DropMath::Lerp(a, b, t); }

} // namespace DropMath